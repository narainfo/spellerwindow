	...
	function designText(idx){
		...
		for(var i=data[idx].errInfo.length-1;i>=0;i--){
			...
			var color = checkColor(data[idx].errInfo[i].correctMethod);
			// console.log("nCorrectMethod = " +
			// data[idx].errInfo[i].correctMethod);
	...
	function redesign() {
	    $("#tdCorrection1stBox .ul").each(function () {
	        ...
	        var correct_word = $("li:nth-child(1) a", correct_item).text(); // 대치어
																			// 목록 중
																			// 젤 위에
																			// 있는
																			// 것을
...
// //////////////////////////////////////////////////////////////////
	...
	function fMakePopupMsg() {
	    ...
	    // strHTML += " <div style='line-height:1.5em'>&nbsp;&nbsp;맞춤법/문법 검사기에
		// 대한 오해 몇 가지를 알려 드립니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;- 300어절밖에 검사 안 한다.<br/>";
	    // strHTML += " &nbsp;&nbsp;&nbsp;: <font color='red'>아닙니다.</font> 검사하는
		// 길이에는 제한이 없습니다. 단, 사용자의 편의를 위해 300어절 단위로 구별하여 찾을 수 있습니다. 검사창 아랫부분에 보면
		// 오른쪽 화살표가 있습니다. 클릭하시면 300자 단위로 차례로 넘어가면서 검사합니다. (곧 1|2|3처럼 해서 선택하실 수
		// 있게 하겠습니다.)<br/><br/>";
// strHTML += " &nbsp;&nbsp;- 수정이 안 된다.<br/>";
// strHTML += " &nbsp;&nbsp;&nbsp;: <font color='red'>아닙니다.</font> 검사 후 오른쪽에서
// 제시하는 대치어 중 적합한 것을 클릭하시면, 그 어절로 대치합니다. 다시 원래 어절로 돌리시려면 원래 어절을 클릭하시면 됩니다. 틀렸지만
// 대치어가 바르지 않으면, 대치어 제시 부분 오른쪽에 연필 모양의 아이콘을 눌러 원하시는 대치어를 입력하시면 고쳐집니다.<br/>";
// strHTML += " &nbsp;&nbsp;&nbsp;고친 내용을 가져다 쓰시려면 '현재 페이지 복사'를 하여 붙여 쓰시면 됩니다. 또
// 음절 개수는 원문 오른쪽 위에서 찾을 수 있습니다. 다른 불편하신 점이 있으시면 연락해 주시기 바랍니다.<br/><br/>";
// strHTML += " &nbsp;&nbsp;마지막으로 아래아한글이나 MS-Word에서 이 검사기를 사용하실 분은 (주)나라인포테크에서
// 사서 쓰실 수 있습니다.<br/>";
// strHTML += " &nbsp;&nbsp;아래아한글용은 아래아한글의 맞춤법 검사기를 대치하여 사용할 수 있으며, MS-Word용도
// 같지만, 오류 종류에 따라 색깔을 달리해 보여줍니다. <br/>";
// strHTML += " &nbsp;&nbsp;법원의 결재시스템에서는 문서 작성 과정에서 이 맞춤법 검사기를 사용하며, 우리나라 대부분
// 언론사도 이 맞춤법 검사기를 기사 작성에 활용합니다. 전자우편(e-mail) 시스템에서 사용하는 기관도
// 있습니다.<br/><br/></div>";
	    // strHTML += " <div style='line-height:1.5em'>&nbsp;&nbsp;2015년 4월
		// 27일부터 '한국어 맞춤법/문법 검사기'의 5.6판을 공개합니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;5.6판은 법률용어, 시사용어, 외래어를 비롯하여 다수 자료가
		// 추가되었습니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;시스템 불안문제를 해결하려고 새로운 서버를 도입했습니다. 1차로 1대를
		// 추가했으며, 조만간 서버 1대를 추가하겠습니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;또 통계적 방법에 기반을 둔 기술을 개발하여 현재 적용 중이며, 8월경에
		// 6.0판으로 공개하겠습니다. <br/><br/>";
	    // strHTML += " &nbsp;&nbsp;혹시 불편하거나 수정하고 싶은 내용이 있으시면, 아래의 연락처로 의견 남겨주시기
		// 바랍니다. <br/><br/>";
	    // strHTML += " &nbsp;&nbsp;'아래아 한글용'도 함께 5.6판이 나왔으며 자동 업데이트는 이번 주 중에
		// 제공될 예정입니다.<br/><br/>";
	    // strHTML += " <center>[구매 문의 및 의견 주실
		// 곳]</center><br/><center>urimal@pusan.ac.kr, 051) 516 -
		// 9268</center><br/>";
	    // strHTML += " <div style='line-height:1.5em'>&nbsp;&nbsp;안녕하십니까? 서비스
		// 장애 시의 대처법을 알려드리려고 합니다.<br/>";
	    // strHTML += " &nbsp;&nbsp;먼저 '한국어 맞춤법/문법 검사기' 사용에 불편을 끼쳐
		// 죄송합니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;현재 본 서버에서 정상적으로 결과를 얻기 힘드시다면 아래의 서버를 이용해주시길
		// 부탁하겠습니다.<br/>";
	    // strHTML += " &nbsp;&nbsp;<center><a
		// href='http://164.125.36.75/PnuSpellerISAPI_201406'><font size='5'>추가
		// 서버</font></a></center></br/>";
	    // strHTML += " &nbsp;&nbsp;해당 '추가 서버'는 서버 상황에 따라 주소가 수시로 바뀔 수 있으니 가능하면
		// 아래의 '우리말 배움터'를 '<font color='red'>즐겨찾기</font>'해놓으시고 '우리말 배움터' 우측의 '한글
		// 맞춤법/문법 검사기' 배너를 사용해서 접속해주시면 감사하겠습니다.<br/>";
	    // strHTML += " &nbsp;&nbsp;<center><a
		// href='http://urimal.cs.pusan.ac.kr'><font size='5'>우리말
		// 배움터</a></font></center></br/>";
	    // strHTML += " &nbsp;&nbsp;다시 한 번, 불편을 끼쳐 죄송합니다.<br/>";
	    // strHTML += " &nbsp;&nbsp;현재 새 인터페이스에 <font color='red'>브라우저 호환성
		// 문제</font>가 있습니다. 검사 결과가 제대로 보이지 않거나 문제가 있으시면 검사창 메인에 있는 <font
		// color='red'>'기존 검사기 가기'</font> 버튼을 클릭하여 5.0 버전의 인터페이스를 사용해주시길
		// 바랍니다.<br/>";
	    // strHTML += " &nbsp;&nbsp;불편을 끼쳐 죄송합니다. 빠른 시일 내에 해결하도록
		// 하겠습니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;사용하시면서 불편하거나 수정하고 싶은 내용이 있으시면, 다음의 메일로 의견
		// 남겨주시기 바랍니다. (urimal@pusan.ac.kr) 새로 바뀐 '한국어 맞춤법/문법 검사기'에도 많은 관심과 좋은
		// 의견 부탁합니다.<br/><br/></div>";
	    // strHTML += " <div style='line-height:1.5em'>&nbsp;&nbsp;8월 9일(토)~8월
		// 10일(일), 접속 장애에 대해 알립니다. <br/><br/>";
	    // strHTML += " &nbsp;&nbsp;2014년 8월 9일 오전 8시부터 8월 10일 오후 6시까지 '고압변전실
		// 수변전설비 설치에 따른 휴전'으로, 우리말 배움터 서버가 있는 곳(부산대학교)의 전기가 공급되지 않아 우리말 배움터에
		// 접속하실 수 없습니다.<br/><br/>";
	    // strHTML += " &nbsp;&nbsp;1. 일시 : 2014-08-09(토) 08:00 ~ 2014-08-10(일)
		// 18:00 <br/>";
	    // strHTML += " &nbsp;&nbsp;2. 대상 : 부산대학교 자연대연구실험동 <br/><br/>";
	    // strHTML += " &nbsp;&nbsp;정전 후 서버를 바로 재가동하여 최대한 이용에 불편함이 없도록 하겠습니다.
		// <br/><br/></div>";
// strHTML += " <div style='line-height:1.5em'>&nbsp;&nbsp;MS Word용 '한국어 맞춤법/문법
// 검사기'의 베타테스터를 모집합니다.<br/><br/>";
// strHTML += " &nbsp;&nbsp;(주)나라인포테크에서는 MS Word용 한국어 맞춤법/문법 검사기를 개발하고
// 있습니다.<br/>";
// strHTML += " &nbsp;&nbsp;현재 버전으로 시험 사용하실 분들의 신청을 받고자 합니다.<br/>";
// strHTML += " &nbsp;&nbsp;신청을 원하시는 분들은 아래 링크를 통해 신청을 해주시면 됩니다.<br/><br/>";
// strHTML += " &nbsp;&nbsp;사용하시면서 불편한 점들에 대해 메일로 보내주시면 검사기 개발에 큰 도움이
// 되겠습니다.<br/><br/>";
// strHTML += " &nbsp;&nbsp;새로 바뀐 '한국어 맞춤법/문법 검사기'에 많은 관심과 좋은 의견 부탁합니다. <br/>";
// strHTML += " &nbsp;&nbsp;현재 사용하시는 온라인 검사기도 새 버전입니다. <br/>";
// strHTML += " &nbsp;&nbsp;새 검사기를 사용하시면서 불편하거나 수정하고 싶은 점이 있으시면, 메일로 의견 보내주시기
// 바랍니다. <br/><br/>";
// strHTML += " &nbsp;&nbsp;상업적인 목적으로 사용하려고 하거나 구매를 하시고자 한다면 나라인포테크로 문의하시면 됩니다.
// <br/>";
// strHTML += " &nbsp;&nbsp;(개인에게 판매하는 한국어 맞춤법/문법 검사기는 '아래아 한글용'입니다.)
// <br/><br/>";
// strHTML += " &nbsp;&nbsp;MS Word 2010 용 한국어 맞춤법/문법 검사기는 내부 테스트용이 있습니다. 테스트해
// 보고 싶으신 분은 아래의 메일로 연락 주시면 됩니다. <br/><br/></div>";
// strHTML += " <br/><center>[신청하시는 곳]</center><br/><center><a
// href='https://goo.gl/forms/P4R4P7blO10tusap2'>https://goo.gl/forms/P4R4P7blO10tusap2</a></center><br/><br/>";
	    ...
	    // strHTML += " &nbsp;&nbsp;[내용]<br/>";
	    // strHTML += " &nbsp;&nbsp;&nbsp;명사구(명사+명사)는 띄어 써야 하지만, 맞춤법 검사기에서는
		// '고유명사·전문용어'와 '일반 명사구'를 구별하기 어려워 띄어 씀과 붙여 씀을 모두 허용합니다. <br/>";
	    // strHTML += " &nbsp;&nbsp;&nbsp;또한, 표준국어대사전에서 '^' 기호는 띄어 씀이 원칙이고 붙여 씀도
		// 허용한다는 뜻입니다. <br/>";
	    // strHTML += " &nbsp;&nbsp;&nbsp;맞춤법 검사기는 이런 단어들이 쓰일 때 한 문서 내에서는 일관성을
		// 유지하고자 처음으로 들어온 단어의 띄어쓰기에 맞춰 교정합니다. <br/>";
	    // strHTML += " &nbsp;&nbsp;&nbsp;예를 들어 '부분 일치'로 먼저 쓰이면, 그 문서에서는 '부분
		// 일치'로 띄어 쓰도록 하고, '부분일치'로 먼저 쓰이면, 그 문서에서는 '부분일치'로 붙여 쓰도록 합니다.<br/>";
	    // strHTML += " &nbsp;&nbsp;&nbsp;해당 오류는 회색으로 표시합니다.<br/><br/>";

	    // strHTML += " </p>";

	    // strHTML += " <center>urimal@pusan.ac.kr <a href='#'
		// onclick='fClosePopupMsg();fShowUserReportPopup(\"./\");'>[의견
		// 보내기]</a></center><br/><br/>";

// strHTML += " &nbsp;&nbsp;<font style='font-weight:normal;'>1년여 동안 많은 고민 끝에 나온
// 시스템입니다. 많은 관심과 좋은 의견 부탁합니다.</font><br/><br/>";
